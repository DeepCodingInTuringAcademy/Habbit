...
{
    // 创建主垂直布局
    auto* mainVLayout = new QVBoxLayout(main_widget);

    // 创建内容区的网格布局（内容将由 refreshMainView 填充）
    main_content_grid_layout = new QGridLayout();
    ...
    // 添加到主垂直布局中
    mainVLayout->addLayout(main_content_grid_layout, 10);
...
{
    if (settings_widget->layout()) return;
